...
> **默认行为（从现在起，不等用户点名）**：判定为实质任务 → **你的第一个动作就是加载 `efw`，先于任何领域工具（pdf/xlsx/脚本…）**。这是一个**具体动作**，不是"有这个心态就行"。
>
> **按「任务意图」判，不按「本次动作」判（关键修正）**：只要请求**指向交付物**（报告/PPT/网页/代码/设计稿…，含 .xlsx/.pptx/.docx/.pdf/.html），**哪怕当下只要建议也算进入"预备态"**——可以先给建议，但必须声明"此为咨询，一旦转产出即加载 efw"，不能用"这步只是给建议"拖延不起壳。
>
> 🔁 **意图升级重触发**：命中「做成 / 导出 / 生成 / 落地 / 帮我做一份 / 直接出 / 转成文件 / 写个…」→ 无论之前是否按咨询处理，**立刻回到本动作加载 efw**。
> 🛡️ **高危类目硬默认**：PPT / 设计 / 文档 / 数据分析 / 报告 默认视为"指向交付物"，直接起壳，不先问"是否实质"。
> 🪶 **lite 轻量模式**：纯咨询且指向交付物时，只做 ①一句话计划 + ③预置验收标准，不必跑全套——起壳成本低了，就没理由为省 token 跳过。
