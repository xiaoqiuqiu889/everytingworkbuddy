...
WorkBuddy 没有 `~/.workbuddy/agents/` 这类自动加载目录，子代理通过 **Agent 工具**（可选 `Explore`/`Plan`/`general-purpose` 等类型）在对话中临时调度。因此这些文件的用法是：
...
通过 `expert-manager` 已把这 9 个子代理转成 WorkBuddy 专家中心可一键拉起的正式专家（命名 `efw-planner` 等），
经 `init → fill → validate → register` 流程，已登记进 `marketplace.json`，专家中心可见。
专家包位于：`~/.workbuddy/plugins/marketplaces/my-experts/plugins/efw-*/`。
