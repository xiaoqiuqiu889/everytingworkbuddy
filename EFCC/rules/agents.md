...
子代理是**独立上下文**——它**读不到** `用户级记忆（Claude: ~/.claude/CLAUDE.md ; Codex: ~/.codex/AGENTS.md）` 里的 EFCC 纪律（那只对主对话生效）。所以派给它的实质任务若希望它守 EFW，**必须把编排外壳摘要写进 prompt**，否则它默认不会用（实测：子 agent 100% 不会主动读用户级 MEMORY）。
...
- 若项目根已放了 `examples/AGENTS.md`（EFCC 编排外壳），子代理会自动读到项目级文件，此时可不重复附——但主动委派时附上更稳。
