...
`catalog/capabilities.json` 是一份**用户价值导向、跨多角色、持续丰富的策展能力库**，已内置标注 **150 条能力**（105 技能 + 10 子代理 + 7 准则 + 9 MCP + 14 专家 + 5 工作流），覆盖研发/游戏/数据金融/研究/写作/设计/产品/营销/办公/通讯/知识管理等维度。每条带 `triggers`（触发词）、`tags`（角色/工作维度）、`activate`（启用方式）。

> **为什么是 150 条、且不基于你本机？** 这份库是按 **WorkBuddy 生态的公开使用热度 + 多角色覆盖**编排的（SkillHub 2.2 万+ 技能里办公协作/开发/数据分析/创意设计四大类使用量最大；企业实测 TOP 技能是批量文件、数据分析+PPT、MCP 自动化、多专家并行、OCR+知识库；高热度技能如 pdf/docx/xlsx/pptx、humanizer-zh、web-search、tencent-meeting 等）。**它代表「广大用户最需要什么」，不是某人本地装了什么**——所以你会看到 pdfkit、OCR-Local、TeamChat Pro、飞书表格等你机器上未必装的生态高热度能力。

但这份清单**不是上限**——`match.mjs` 会再**动态发现你机器上已安装的全部技能**（`~/.workbuddy/skills/*`，含 WorkBuddy 内置技能）并解析其 `SKILL.md` 自动纳入检索，且**随你安装新技能自增长**。想确定性排序时也可直接跑：
