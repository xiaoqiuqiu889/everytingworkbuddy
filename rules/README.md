...
WorkBuddy 没有自动加载 `rules/` 目录的机制。让这些准则真正生效的方式：

1. **并入用户级 `MEMORY.md`（推荐，全局生效）**：把每份准则的「核心红线」浓缩后写入 `~/.workbuddy/MEMORY.md`，每个会话自动加载。EFW 已为你做好这一步（见 install.md）。
2. **并入项目级记忆**：针对特定项目的额外约束，写入该项目的 `.workbuddy/memory/MEMORY.md`。
