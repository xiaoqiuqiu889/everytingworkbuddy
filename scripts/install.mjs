...
**什么算"实质任务"——按「任务意图指向什么」判，不按「本次动作是否产文件」判（关键修正）**：
- ① **指向交付物**：请求指向一个交付物（报告/PPT/网页/代码/设计稿…，含 .xlsx/.pptx/.docx/.pdf/.html）——**哪怕当下只要建议、不直接产文件，也算进入"预备态"**。咨询类若指向交付物，可以先给建议，但必须显式声明"此为咨询，一旦转产出即加载 efw"；禁止用"这步只是给建议"无限拖延不起壳。
- ② 需要 **≥3 步**；或 ③ 预计 **≥15 次工具调用**。
- 简单问答 / 查一条资料 / 纯闲聊（**不指向任何交付物**）才算"非实质"。

> 🔁 **意图升级 → 重新触发 0 号动作**：一旦命中升级信号词——「做成 / 导出 / 生成 / 落地 / 帮我做一份 / 直接出 / 转成文件 / 写个…」——**无论之前是否按咨询处理，立刻回到 0 号动作加载 efw**。咨询转产出是明确的起壳信号，不顺口带过。

> 🛡️ **高危类目硬默认（不靠自觉）**：PPT / 设计 / 文档 / 数据分析 / 报告 是已知高频漏用区。**这些类目默认视为"指向交付物"，直接进 0 号动作**，不要先问"是不是实质任务"——把举证责任翻过来：除非明显是纯闲聊，否则就起壳。

> 🪶 **efw 轻量模式（lite，降低起壳成本）**：纯咨询且指向交付物时，不必跑全套——只做 **①一句话计划（打算做成什么）+ ③预置验收标准**；不强制完整外壳。成本低了，就没理由为省 token 跳过。
